...
dataset = torchvision.datasets.CIFAR10('E:/Data',
                                    train=True,
                                    transform=transform,
                                    download=True)
...
# 构造网络
generator = gan.GeneratorCIFAR10().to(device)
discriminator = gan.DiscriminatorCIFAR10().to(device)
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 10 == 0 or epoch == 0:
        ...
        for i in range(16):
            plt.subplot(4, 4, i + 1)
            # 先归一化，再转换格式
            plt.imshow((predict[i] + 1) / 2)  # CIFAR10是RGB图像,不需要cmap
            plt.axis('off')
        plt.savefig(f"./generated_img/generated_image_epoch_{epoch+1}.png")
        plt.close()
